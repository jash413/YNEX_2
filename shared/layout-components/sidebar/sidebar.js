...
import MenuItems, { FooterMenuItems } from "./nav";
			...
			menuitems.filter(mainlevel => {
				if (mainlevel.Items) {
					mainlevel.Items.filter((items) => {
						...
						if (dd === "") {
							dd = "/components/business-hub/summary/";
						}
						if (dd === items.path + "") {
							items.active = true;
							items.selected = true;
		...
		<Fragment>
			...
			<aside className="app-sidebar sticky" id="sidebar" onMouseOver={() => Onhover()}
				...
				<div className="main-sidebar-header">
					<Link href="/components/business-hub/summary/" className="header-logo">
...
{/* 
						<div style={{ position: "absolute", bottom: 80, width: "100%" }}><h1>xxxxx</h1></div>		 */}
